...
main:
	...
	#li $a0, 10                     #$a0 = tamanho do array
	li $a1, 0                      #$a1 = tipo 0 == selection | 1 == quick
	#la $a2, array                  #$a2 = endereço inicio do array
	
	jal ordena                     #chama ordena
	
	
	#####
	##### tem q guardar as coisas aki pra n cagar o resto :)
	#####
	
	la $t3, var
	
	li $t0, 0
	move $t1, $v1 ## tem q guardar 
	grava:
       	 	beq $t1, $t0, endGrava
       	 	lb $t2, ($s0) ## carrega o 1ro byte
	...
	addi $sp, $sp, 4               #remove 1 espaço na pilha
...
##################### SELECTION SORT ##########################################	
selection_sort:
	addi $sp, $sp, -32             #abre 8 espaços na pilha
